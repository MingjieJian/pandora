     ...
     $(KILROY,CALLER)
C
C     Rudolf Loeser, 2002 Oct 08
C---- Checks consistency of key parameter statements.
C     !DASH
      save
C     !DASH
      integer LUEO
...
C
      call HI ('AVA')
C     !BEG
      if(LIMIT.ne.MILTY) then
        call MUSHED (CALLER, 1, KILROY)
        write (LUEO,100)
  100   format(' ','MILTI(Index) .ne. MILTY(Levdes): recompile.')
      end if
      if(LIMIT.ne.MXLEV) then
        call MUSHED (CALLER, 1, KILROY)
        write (LUEO,101)
  101   format(' ','MILTI(Index) .ne. MXLEV(Limbo): recompile.')
      end if
      if(LIMIT.ne.MAXLV) then
        call MUSHED (CALLER, 1, KILROY)
        write (LUEO,102)
  102   format(' ','MILTI(Index) .ne. MAXLV(Thule): recompile.')
      end if
C     !END
      call BYE ('AVA')
C
