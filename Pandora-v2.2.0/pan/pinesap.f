      subroutine PINESAP
     $(IPOP,BCKSM,KOPAC,XLM,XLP,UPO)
C
C     Rudolf Loeser, 1988 Nov 01
C---- Sets UPO = .true. if the bound-free opacity components
...
C     !DASH
      ...
      integer IN, IPOP, KOPAC
      logical NEWAVE, NEWBD, NEWPOP, NEWTE, UPO
C     !COM
...
C     !BEG
      ...
      if(UPO) then
        IN     = ICKSM(IPOP)
        NEWPOP = BCKSM(IN  ).ne.CSBA(IN  )
        NEWBD  = BCKSM(IN+1).ne.CSBA(IN+1)
        NEWTE  = BCKSM(   1).ne.CSBA(   1)
        NEWAVE = XLM.ne.XLP
        UPO    = NEWPOP.or.NEWBD.or.NEWTE.or.NEWAVE
      end if
C     !END
      call BYE ('PINESAP')
C
