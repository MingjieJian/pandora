...
C     background He-II lines, for TRAVIS.
C     (This is version 2 of TRISH.)
C     !DASH
      save
C     !DASH
...
C     !BEG
      if(KOPAC(33).gt.0) then
        K = 0
        call HAWERA     (X, 1, 0, NIAUGM, DS, K)
        do 100 I = 1,K
          call STRIVA   (DS(I), ZERO, WLO, WHI, NTMX, TAB, NT, 'TRISH')
  100   continue
      end if
C     !END
      call BYE ('TRISH')
