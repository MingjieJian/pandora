...
C---- Determines whether of not to dump, for ROSE; and writes a dump
C     header if yes.
C     (This is version 2 of VETIVER.)
C
C     See also SANDBUR.
C     !DASH
      save
...
C     !BEG
      DUMP = .false.
C
      if(((IPPOD.eq.2).or.(IPPOD.eq.3)).and.(KONLUD.gt.0)) then
...
C
  ...
  100     format(' ','DUMP of emission contributions from each level ',
     $               'of population ion #',I3,' at depth #',I4,
     $               ' and Lambda =',1PE20.13/
     $           ' ',12X,'BD',10X,'Te',4X,'h*nu/k*T',3X,'exp(-...)',11X,
     $               'H',10X,'XLMTHR',10X,'PILEVL',4X,'num sum term',4X,
     $               'den sum term')
        end if
      end if
C     !END
