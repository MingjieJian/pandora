...
C---- Reads part(s) of a Line Intensity Data Block.
C     (See also LIDPUT and LIDMAK.)
C     !DASH
      save
C     !DASH
...
C     !DASH
C     !EJECT
      external LEYTE, LIDCEK, MESHED, ABORT, HI, BYE
C
...
C     !BEG
      if((INDEX.lt.1).or.(INDEX.gt.NUMTRN)) then
        call MESHED ('LIDGET', 1)
        write (LUEO,100) INDEX,NUMTRN,K1,K2,K3
  100   format(' ','Error reading Line Intensity Data Blocks.'/
     $         ' ','INDEX = ',I10,2X,'(NUMTRN = ',I10,')',5X,3I5)
        call ABORT
      end if
C
      if(K1.gt.0) then
        call LEYTE  (XLB1, LI1LEN, LI1ADR(INDEX))
        call LIDCEK ('LIDGET', 1, XLB1(MMNAM), LINNAM(INDEX))
      end if
C
      if(K2.gt.0) then
        call LEYTE  (XLB2, LI2LEN, LI2ADR(INDEX))
        call LIDCEK ('LIDGET', 2, XLB2(MMYAM), LINNAM(INDEX))
      end if
C
      if(K3.gt.0) then
        call LEYTE  (XLB3, LI3LEN, LI3ADR(INDEX))
        call LIDCEK ('LIDGET', 3, XLB3(MMZAM), LINNAM(INDEX))
      end if
C     !END
      call BYE ('LIDGET')
C
