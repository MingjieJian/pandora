     ...
     $(XLM,BCKSM)
C
C     Rudolf Loeser, 1988 Nov 07
C---- Prints continuum contributions control debug output.
C     (This is version 2 of FULGOR.)
C     !DASH
      save
C     !DASH
      real*8 BCKSM, XLM
      integer I, K, KAPDB, LUEO
C     !COM
...
C     !DASH
      external DACAPO, LINER, MESHED, MASHED, HI, BYE
C
C               BCKSM(NCSBA)
      dimension BCKSM(*)
C
      call HI ('FULGOR')
C     !BEG
      if((KAPDB.gt.0).and.(KONLUD.gt.0)) then
        call DACAPO (XLM)
        call MESHED ('FULGOR', 2)
C
        call LINER  (1, LUEO)
        write (LUEO,100) XLM
  100   format(' ','Wavelength =',1PE24.16,': atmosphere data ',
     $             'checksums comparison.')
        call LINER  (1, LUEO)
C
        K = 0
        do 102 I = 1,NCSBA
          if(CSBA(I).ne.BCKSM(I)) then
            K = K+1
            write (LUEO,101) I,CSBA(I),I,BCKSM(I),LABSBA(I)
  101       format(' ',3X, 'CSBA(',I2,') =',1PE24.16,
     $                 5X,'BCKSM(',I2,') =',  E24.16,5X,A)
          end if
  102   continue
        if(K.le.0) then
          write (LUEO,103)
  103     format(' ',3X,'All identical')
        end if
C
        call MASHED ('FULGOR')
      end if
