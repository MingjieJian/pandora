...
C---- Sets up rotational CO lines, for WEAVE.
C     !DASH
      save
C     !DASH
      real*8 CA, FA, dummy
...
C
        if(KMX.gt.0) then
          do 101 K = 1,KMX
            do 100 J = 1,JMX
              if(COMPW) then
                call COWVNUM (J-1,J,K-1,K-1,ISO,WNROT(J,K,I),PRNT,
     $                        METHCOW,dummy)
              end if
              if(COMPF) then
                call COFVAL  (J-1,J,K-1,K-1,ISO,WNROT(J,K,I),RC1213,
     $                        FA,CA,PRNT,METHCOF,FROT(J,K,I))
              end if
  100       continue
  101     continue
C
          if(LU.gt.0) then
            call LINER       (2,LU)
            write (LU,102) ISO
  102       format(' ','Rotational CO lines: wavenumbers and ',
     $                 'oscillator strengths, for Carbon(',I2,')')
            call DISKO       (LU,WNROT(1,1,I),FROT(1,1,I),JCOLMR,
     $                        JMX,KMX,0,1)
          end if
        end if
  103 continue
