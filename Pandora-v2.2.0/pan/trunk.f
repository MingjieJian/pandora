...
C     background He-I lines, for TRAVIS.
C     (This is version 4 of TRUNK.)
C     !DASH
      save
C     !DASH
...
C     !BEG
      if(KOPAC(35).gt.0) then
        K = 0
        call QUAHOG   (X, 1, 0, NIAUGM, DS, K)
        do 100 I = 1,K
          call STRIVA (DS(I), ZERO, WLO, WHI, NTMX, TAB, NT, 'TRUNK')
  100   continue
      end if
C     !END
      call BYE ('TRUNK')
