      subroutine ERLING
C
C     Rudolf Loeser, 1994 Aug 12
C---- Computes second overtone CO lines opacity.
C     !DASH
      save
C     !DASH
      real*8 ONE, XCOMX
...
C----       UP transitions: j -> j+1
            JUD  = +1
            YES1 = WLM(M).le.WNUPS(I,1,M)
            YESK = WLP(M).ge.WNUPS(I,KCOSEC(M),M)
            YES  = YES1.and.YESK
            if(YES) then
              do 100 L = 1,KCOSEC(M)
                AX = abs(C1(M)*((C2*WNUPS(I,L,M)-ONE)+SHFT))
                if(AX.lt.XCOMX) then
                  call FLOUR (WNUPS(I,L,M), FUPS(I,L,M), EXEN(I,L,M))
                end if
  100         continue
            end if
C
C----       DOWN transitions: j -> j-1
            JUD  = -1
            YES1 = WLM(M).le.WNDNS(I,1,M)
            YESK = WLP(M).ge.WNDNS(I,KCOSEC(M),M)
            YES  = YES1.and.YESK
            if(YES) then
              do 101 L = 1,KCOSEC(M)
                AX = abs(C1(M)*((C2*WNDNS(I,L,M)-ONE)+SHFT))
                if(AX.lt.XCOMX) then
                  call FLOUR (WNDNS(I,L,M), FDNS(I,L,M), EXEN(I+1,L,M))
                end if
  101         continue
            end if
C
