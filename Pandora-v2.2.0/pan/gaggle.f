      subroutine GAGGLE
     $(N,ML,TE,IPOP,POP,LIMP)
C
C     Rudolf Loeser, 1972 Nov 14
C---- Computes the missing Population Densities for levels L,
C     NL .lt. L .le. LIMP, where 0 .lt. NL .lt. LIMP.
C     !DASH
      save
C     !DASH
      real*8 POP, R, TE
      integer I, IPOP, J, LIMP, ML, N, NL
C     !COM
...
C     !BEG
      if(ML.lt.LIMP) then
        ...
        do 101 J = (NL+1),LIMP
          do 100 I = 1,N
            call ERGOT (TE(I), XLMTHR(NL,IPOP), XLMTHR(J,IPOP),
     $                         PILEVL(NL,IPOP), PILEVL(J,IPOP), R)
            POP(I,J) = R*POP(I,NL)
  100     continue
  101   continue
      end if
