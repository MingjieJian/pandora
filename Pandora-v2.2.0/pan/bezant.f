     ...
     $(N,K,SF,TIT,IMG,FO)
C
C     Rudolf Loeser, 1982 Jan 19
C---- Edits SF, for PRD.
C     !DASH
      save
C     !DASH
      real*8 FO, SF, ZERO
      integer IL, IMG, IQQSE, IU, J, K, KMSS, MO, N, NERM, jummy
      logical BAD, KILROY
      character LAB*80, TIT*(*), qummy*1
C     !COM
...
C
        KILROY = .true.
        do 101 J = 1,K
          write (LAB,100) TIT,IU,IL,J
  100     format('PRD modified Source Function, ',A,', for transition ',
     $           I2,'/',I2,5X,'Frequency No. ',I4)
C
          call EDITH   (SF(1,J), N, ZERO, 1, 2, KMSS, LAB, IMG, FO,
     $                  KERMED(3), NERM, BAD)
C
          if(BAD.and.(KMSS.le.0)) then
            call WOBLE (IMG, N, LAB, 'BEZANT', KILROY, 2)
          end if
  101   continue
        call WOBLE     (jummy, jummy, qummy, 'BEZANT', KILROY, 3)
C
      end if
C     !END
      call BYE ('BEZANT')
