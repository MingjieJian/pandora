...
C---- Finds a block index in an unsorted Continuum Blocks index.
C     !DASH
      save
C     !DASH
      real*8 PWAVE
...
C     !DASH
C     !EJECT
      external LOOKUD, HALT, HI, BYE
C
      call HI ('PRIONE')
C     !BEG
      call LOOKUD (CONWAV, NUMKON, WAVEDEL, PWAVE, K, LOOK)
      if(LOOK.eq.2) then
        write (MSSLIN(1),100) PWAVE
  100   format('PWAVE =',1PE20.12,'; but not found in unsorted CONWAV ',
     $         'table.')
        call HALT ('PRIONE', 1)
      end if
C     !END
