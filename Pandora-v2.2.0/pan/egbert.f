...
C---- Computes first overtone CO lines opacity.
C     !DASH
      save
C     !DASH
      real*8 ONE, XCOMX
...
C----       UP transitions: j -> j+1
            JUD  = +1
            YES1 = WLM(M).le.WNUPO(I,1,M)
            YESK = WLP(M).ge.WNUPO(I,KCOOVR(M),M)
            YES  = YES1.and.YESK
            if(YES) then
              do 100 L = 1,KCOOVR(M)
                AX = abs(C1(M)*((C2*WNUPO(I,L,M)-ONE)+SHFT))
                if(AX.lt.XCOMX) then
                  call FLOUR (WNUPO(I,L,M), FUPO(I,L,M), EXEN(I,L,M))
                end if
  100         continue
            end if
C
C----       DOWN transitions: j -> j-1
            JUD  = -1
            YES1 = WLM(M).le.WNDNO(I,1,M)
            YESK = WLP(M).ge.WNDNO(I,KCOOVR(M),M)
            YES  = YES1.and.YESK
            if(YES) then
              do 101 L = 1,KCOOVR(M)
                AX = abs(C1(M)*((C2*WNDNO(I,L,M)-ONE)+SHFT))
                if(AX.lt.XCOMX) then
                  call FLOUR (WNDNO(I,L,M), FDNO(I,L,M), EXEN(I+1,L,M))
                end if
  101         continue
            end if
C
