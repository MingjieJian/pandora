...
C
C     Rudolf Loeser, 1989 Feb 16
C---- Computes LCOW, the number of CO lines in the specified spectrum
C     ranges, i.e. the number of actual CO line cores included.
C     !DASH
      save
C     !DASH
      real*8 WNA, WNB, XLCOA, XLCOB
      integer IPEX, ISO, J, LCOW, LUEO, NCB, NDN, NUP
      logical DUMP
C     !COM
...
C     !DASH
      external WANDA, MAYWEED, LINER, MESHED, MASHED, HI, BYE
C
...
C
      do 103 J = 1,NCB
        call WANDA       (XLCOA(J),WNB)
        call WANDA       (XLCOB(J),WNA)
        NUP = 0
        NDN = 0
C
        do 100 ISO = 1,2
          if(ROTACO(ISO)) then
            call MAYWEED (JCOROT(ISO), KCOROT(ISO), JCOLMR, KCOLMR,
     $                    'WNROT', WNROT(1,1,ISO), WNA, WNB, NUP, DUMP)
          end if
          if(FUNDCO(ISO)) then
            call MAYWEED (JCOFUN(ISO), KCOFUN(ISO), JCOLMF, KCOLMF,
     $                    'WNUPF', WNUPF(1,1,ISO), WNA, WNB, NUP, DUMP)
            call MAYWEED (JCOFUN(ISO), KCOFUN(ISO), JCOLMF, KCOLMF,
     $                    'WNDNF', WNDNF(1,1,ISO), WNA, WNB, NDN, DUMP)
          end if
          if(OVERCO(ISO)) then
            call MAYWEED (JCOOVR(ISO), KCOOVR(ISO), JCOLMO, KCOLMO,
     $                    'WNUPO', WNUPO(1,1,ISO), WNA, WNB, NUP, DUMP)
            call MAYWEED (JCOOVR(ISO), KCOOVR(ISO), JCOLMO, KCOLMO,
     $                    'WNDNO', WNDNO(1,1,ISO), WNA, WNB, NDN, DUMP)
          end if
          if(SECNCO(ISO)) then
            call MAYWEED (JCOSEC(ISO), KCOSEC(ISO), JCOLMS, KCOLMS,
     $                    'WNUPS', WNUPS(1,1,ISO), WNA, WNB, NUP, DUMP)
            call MAYWEED (JCOSEC(ISO), KCOSEC(ISO), JCOLMS, KCOLMS,
     $                    'WNDNS', WNDNS(1,1,ISO), WNA, WNB, NDN, DUMP)
          end if
  100   continue
C
        LCOW = LCOW+NUP+NDN
C
        if(DUMP) then
          call LINER     (1, LUEO)
          write (LUEO,101) ROTACO,FUNDCO,OVERCO,SECNCO
  101     format(' ','ALISMA',5X,4(L4,L3))
          write (LUEO,102) XLCOB(J),WNA,XLCOA(J),WNB,NDN,NUP,LCOW
  102     format(' ','ALISMA',5X,'LCOB=',1PE10.3,5X,'WNA=',E10.3,5X,
     $               'LCOA=',E10.3,5X,'WNB=',E10.3,5X,'NDN=',I4,5X,
     $               'NUP=',I4,5X,'LCOW=',I4)
          call MASHED    ('ALISMA')
        end if
  103 continue
