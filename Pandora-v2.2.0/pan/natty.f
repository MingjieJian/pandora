...
C---- Plays with data needed for O-III background contributor lines.
C     !DASH
      save
C     !DASH
      real*8 DNU, W, X
...
C---- Set up central wavelengths
      do 100 I = 1,MX3L
        DNU = X3NUU(I)-X3NUL(I)
        call ANGIE (DNU, X3WVL(I))
  100 continue
C---- Set up wavelength cut-offs
      call DIREX   (X(JJTE), X(JJV), X(JJBXI), X3MAS, X3WVL, X3WLO,
     $              X3WHI, MX3L)
C---- Modify for blends
      call RITTER  (MX3L, LX3L, LDLX3, X3DDL, X3WLO, X3WHI)
C     !END
      call BYE ('NATTY')
C
      return
