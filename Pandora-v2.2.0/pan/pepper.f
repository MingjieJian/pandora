     ...
     $(LU)
C
C     Rudolf Loeser, 1994 Aug 11
C---- Sets up second overtone CO lines, for WEAVE.
C     (This is version 2 of PEPPER.)
C
C     As of August 1994, we have no computational procedures
C     for replacing the built-in default Goorvitch values.
C     Thus, the only purpose of PEPPER is printing.
C     !DASH
      save
C     !DASH
      integer I, ISO, ISOTOPE, JMX, KMX, LU
C     !COM
...
C----       For "up"
            call LINER (2,LU)
            write (LU,100) ISO
  100       format(' ','Second overtone CO lines: wavenumbers and ',
     $                 'oscillator strengths, for Carbon(',I2,')')
            call DISKO (LU,WNUPS(1,1,I),FUPS(1,1,I),JCOLMS,JMX,KMX,3,1)
C
C----       For "down"
            call LINER (2,LU)
            write (LU,100) ISO
            call DISKO (LU,WNDNS(1,1,I),FDNS(1,1,I),JCOLMS,JMX,KMX,3,2)
          end if
  101   continue
C
      end if
