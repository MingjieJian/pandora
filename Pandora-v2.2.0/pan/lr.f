...
C---- Computes the term LR, for the RT weight matrix.
C     !DASH
      save
C     !DASH
      real*8 DT, E2J, E3J, E3JM, EA3, HALF, HNDRD, HNDRDTH, RAT, SE2,
     $       SE3, SES2, SES3, TERM, TM, TWO, ZERO
      integer J, JM, KODE
C     !COM
...
C     !DASH
      external ASSUR, HICKORY, WALNUT, DIVIDE, HI, BYE
C
...
C
        KODE=1
C
        if((DT(J).gt.HNDRD*TM).and.(TM.lt.HNDRDTH)) then
          call ASSUR     (DT(J),-TM,3,EA3)
          call DIVIDE    (EA3,(TWO*TM),TERM)
C
        else
C
          if(DT(JM).lt.HNDRDTH) then
