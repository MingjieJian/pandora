...
C---- Computes rotational CO lines opacity.
C     !DASH
      save
C     !DASH
      real*8 ONE, XCOMX
...
C----       UP transitions: j -> j+1
            JUD  = +1
            YES1 = WLM(M).le.WNROT(I,1,M)
            YESK = WLP(M).ge.WNROT(I,KCOROT(M),M)
            YES  = YES1.and.YESK
            if(YES) then
              do 100 L = 1,KCOROT(M)
                AX = abs(C1(M)*((C2*WNROT(I,L,M)-ONE)+SHFT))
                if(AX.lt.XCOMX) then
                  call FLOUR (WNROT(I,L,M), FROT(I,L,M), EXEN(I,L,M))
                end if
  100         continue
            end if
C
