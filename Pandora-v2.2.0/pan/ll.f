...
C---- Computes the term LL, for the RT weight matrix.
C     !DASH
      save
C     !DASH
      real*8 DT, E2J, E3J, E3JP, EA3, HALF, HNDRD, HNDRDTH, RAT, SE2,
     $       SE3, SES2, SES3, TERM, TP, TWO, ZERO
      integer J, JP, KODE
C     !COM
...
C
        KODE = 1
        if((DT(J).gt.HNDRD*TP).and.(TP.lt.HNDRDTH)) then
          call ASSUR     (DT(J),TP,3,EA3)
          call DIVIDE    (EA3,(TWO*TP),TERM)
C
        else
C
