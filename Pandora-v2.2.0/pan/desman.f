     ...
     $(FHZ,K)
C
C     Rudolf Loeser, 1984 Dec 07
C---- Saves checksum of computed Flux.
C     !DASH
      save
C     !DASH
      real*8 FHZ
...
C
      call HI ('DESMAN')
C     !BEG
      write (TIT,100) LFB,NVY,IU,IL
  100 format(I3,I2,' (',I2,'/',I2,') Profile : Flux',12X)
C
      call CHECKER (FHZ,1,K,TIT)
C     !END
      call BYE ('DESMAN')
C
      return
