      subroutine OXMORON
     $(X,W,XPBL1,K1,XPBL2,K2,XPBL3,K3,KODE)
C
C     Rudolf Loeser, 2007 Jan 15
C---- Does the initial processing of "population data"
C     of ions with three stages of ionization.
C     !DASH
      save
C     !DASH
      real*8 W, X, XPBL1, XPBL2, XPBL3, dummy
      integer IABDL, ID1, ID2, ID3, IN, INDX1, INDX2, INDX3, IRAB, IS,
     $        IX1, IX2, IX3, JJBDI, JJCHI, JJHND, JJNK, JJPFT, JJRAB,
     $        JJTE, JJXND, JJXNE, K1, K2, K3, KLTE1, KLTE2, KLTE3, KODE,
     $        LC1, LC2, LC3, LLBD, LLPOPK, LLPOPN, MOX, N, NL, NO
      logical PRINTB, PRINTN, RUNION1, RUNION2, RUNION3
C     !COM
...
C     !DASH
C     !EJECT
      external  BELLY, GOGGLE, PATROL, RABBIT, OLM, ARGIA, SERF, WGIVE,
     $          CALVIN, PUPPET, HI, BYE
      intrinsic min
C
      dimension X(*), W(*)
C
C               XPBL1(Lenpbl), XPBL2(Lenpbl), XPBL3(Lenpbl)
      dimension XPBL1(*),      XPBL2(*),      XPBL3(*)
C
      dimension IN(8)
      equivalence
     $(IN( 1),IX1   ),(IN( 2),IX2   ),(IN( 3),ID1   ),(IN( 4),ID2   ),
     $(IN( 5),IRAB  ),(IN( 6),IABDL ),(IN( 7),IX3   ),(IN( 8),ID3   )
C
      call HI ('OXMORON')
C     !BEG
      if(KODE.gt.0) then
...
C       (Get, and allocate, W allotment)
        call BELLY  (IN, IS, MOX, 'OXMORON')
C
        RUNION1 = (IUPOP(K1).gt.0)
        RUNION2 = (IUPOP(K2).gt.0)
        RUNION3 = (IUPOP(K3).gt.0)
C----   Set up RAB table
        call RABBIT ((RUNION1.or.RUNION2.or.RUNION3), N, X(JJRAB),
     $               W(IRAB))
C----   Set up elemental abundance table
        call PATROL (X, POPSYM(K1), INDX1, POPSYM(K2), INDX2,
     $               POPSYM(K3), INDX3, W(IABDL))
C----   Set up population data
        call CALVIN (N, X(JJTE), X(JJXNE), X(JJCHI), X(JJPFT),
     $               X(JJHND), W(IRAB), W(IABDL),
     $               INDX1, LENPOP(K1), XPBL1(LLPOPN), XPBL1(LLPOPK),
     $               KLTE1, K1, LIMPOP(K1), W(IX1), W(ID1),
     $               INDX2, LENPOP(K2), XPBL2(LLPOPN), XPBL2(LLPOPK),
     $               KLTE2, K2, LIMPOP(K2), W(IX2), W(ID2),
     $               INDX3, LENPOP(K3), XPBL3(LLPOPN), XPBL3(LLPOPK),
     $               KLTE3, K3, LIMPOP(K3), W(IX3), W(ID3))
C----   Set up departure coefficient data
     ...
     $               LC2, 0, dummy)
        call GOGGLE (N, LENPOP(K3), LIMPOP(K3), POPSYM(K3), X(JJTE),
     $               X(JJXNE), X(JJPFT), X(JJCHI), W(IABDL),
     $               K3, XPBL3(LLPOPN), XPBL3(LLPOPK), XPBL3(LLBD),
     $               LC3, 0, dummy)
C     !EJECT
...
C----   Print for upper stage
        call ARGIA (PRINTN, PRINTB, RUNION3, LIMPOP(K3), N, X(JJNK),
     $              X(JJXND), X(JJBDI), XPBL3(LLPOPK), XPBL3(LLPOPN),
     $              XPBL3(LLBD))
        call SERF  (NO, LENPOP(K3), N, X(JJHND), X(JJXNE),
     $              XPBL3(LLPOPN), XPBL3(LLPOPK), KLTE3, NAMES(K3),
     $              NAMKNT(K3), LIMPOP(K3), K3, IPSWICH(K3),
     $              XPBL3(LLBD), LC3, 1, PRINTN, PRINTB)
        if(RUNION1) then
        ...
        if(RUNION3) then
          LENPOP(K3) = min(NL,LIMPOP(K3))
        end if
C
C       (Give back W allotment)
        call WGIVE (W, 'OXMORON')
C     !EJECT
      else
C----   No need to compute - just set =0
     ...
     $             XPBL2(LLBD))
        call OLM  (N, LIMPOP(K3), XPBL3(LLPOPK), XPBL3(LLPOPN),
     $             XPBL3(LLBD))
      end if
C---- Checksums
     ...
     $             XPBL2(LLPOPN), 1, XPBL2(LLBD))
      call PUPPET (ICKSM(K3), N, LIMPOP(K3), XPBL3(LLPOPK),
     $             XPBL3(LLPOPN), 1, XPBL3(LLBD))
C     !END
      call BYE ('OXMORON')
C
      return
