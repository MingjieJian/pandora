      subroutine FIZMO
     $(N,X,XNE,ZTRM,FTC,FMC)
C
C     Rudolf Loeser, 1972 Sep 07
C     Revised RL/SGK Apr  9 2014 
C---- Computes FMC - contribution of electrons from metals in the
C                    table, relative to the total electron density
C              FTC - sum of FMC
C     (This is version 2.1 of FIZMO.)
C     !DASH
      save
C     !DASH
      real*8 ZTRM, FMC, FTC, X, XNE, ZERO, dummy
      integer I, L, N
C     !COM
...
C     !DASH
C     !EJECT
      external ROWSUM, ZERO1, HI, BYE
C
      dimension X(*)
C
C               XNE(N), ZTRM(N,NMT), FMC(N,NMT), FTC(N)
      dimension XNE(*), ZTRM(N,*),   FMC(N,*),   FTC(*)
C
      call HI ('FIZMO')
C     !BEG
      call ZERO1      (FMC,(N*NMT))
C
      do 101 L = 1,NMT
        if(LATEM(L)) then
          do 100 I = 1,N
            FMC(I,L) = ZTRM(I,L)/XNE(I)
 100      continue
C
        end if
  101 continue
C
      do 102 I = 1,N
        call ROWSUM   (FMC(I,1),N,N,1,NMT,FTC(I))
  102 continue
C     !END
      call BYE ('FIZMO')
C
