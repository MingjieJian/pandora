...
C---- Sets up first overtone CO lines, for WEAVE.
C     (This is version 2 of WYSTAN.)
C     !DASH
      save
C     !DASH
      real*8 CA, CF, dummy
...
C     .
C     !DASH
      external RIGEL, COWVNUM, COFVAL, LINER, DISKO, HI, BYE
C
      dimension ISOTOPE(2)
C
...
C
        if(KMX.gt.0) then
          do 101 K = 1,KMX
            do 100 J = 1,JMX
              if(COMPW) then
                call COWVNUM (J-1,J,K-1,K+1,ISO,WNUPO(J,K,I),PRNT,
     $                        METHCOW,dummy)
                call COWVNUM (J,J-1,K-1,K+1,ISO,WNDNO(J,K,I),PRNT,
     $                        METHCOW,dummy)
              end if
              if(COMPF) then
                call COFVAL  (J-1,J,K-1,K+1,ISO,WNUPO(J,K,I),RC1213,
     $                        CF,CA,PRNT,METHCOF,FUPO(J,K,I))
                call COFVAL  (J,J-1,K-1,K+1,ISO,WNDNO(J,K,I),RC1213,
     $                        CF,CA,PRNT,METHCOF,FDNO(J,K,I))
              end if
  100       continue
  101     continue
          if(LU.gt.0) then
C----       For "up"
            call LINER       (2,LU)
            write (LU,102) ISO
  102       format(' ','First overtone CO lines: wavenumbers and ',
     $                 'oscillator strengths, for Carbon(',I2,')')
            call DISKO       (LU,WNUPO(1,1,I),FUPO(1,1,I),JCOLMO,
     $                        JMX,KMX,2,1)
C----       For "down"
            call LINER       (2,LU)
            write (LU,102) ISO
            call DISKO       (LU,WNDNO(1,1,I),FDNO(1,1,I),JCOLMO,
     $                        JMX,KMX,2,2)
          end if
        end if
  103 continue
