     ...
     $(LIST,L,N,DIDH)
C
C     Rudolf Loeser, 2006 Nov 06
C---- Saves dI/dh in the Special Spectrum Save File.
C     (This is version 2 of CREMY.)
C     !DASH
      save
C     !DASH
      real*8 DIDH
      integer I, IL, IU, J, K, L, LIST, LUSO, N
C     !COM
...
C
C               DIDH(N,KM), LIST(KM)
      dimension DIDH(N,*),  LIST(*)
C
      call HI ('CREMY')
C     !BEG
      write (LUSO,100) IU,IL,EMOO
  100 format('----8  DI/DH'/ 1P,
...
C
      do 103 K = 1,L
        J = LIST(K)
        write (LUSO,101) J
  101   format(I10,2X,'K, delta-lambda index in DIDH(n,k)')
        write (LUSO,102) (DIDH(I,J),I=1,N)
  102   format(1P10E12.5)
  103 continue
C     !END
      call BYE ('CREMY')
C
