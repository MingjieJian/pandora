...
C
C---- "HELENAR" computes a table of integrals such that
...
C     !DASH
      save
C     !DASH
      real*4 A, D, F, HALF, S, SIX, SUM, T1, T2, T3, T4, THREE, TWO, X,
     $       ZERO
      integer INCA, INCF, INCX, IRET, J, JA, JF, JX, N, NA, NF, NMO, NX
C     !DASH
      external PARBOLR, ABORT
C
      dimension X(*), F(*), A(*), D(3), S(3)
C
      data ZERO,HALF,TWO,THREE,SIX /0.E0, 0.5E0, 2.E0, 3.E0, 6.E0/
C     !EJECT
C
C     !BEG
      if(N.gt.0) then
        ...
        if(N.gt.1) then
          ...
          if(N.ge.3) then
            ...
            do 101 J = 3,NMO
              ...
              JF = JF+INCF
              JA = JA+INCA
              call PARBOLR (X,INCX,F,INCF,J-1,D,S,IRET)
              if(IRET.eq.2) then
                ...
                if(IRET.eq.3) then
                  ...
                  write (*,100) IRET
  100             format(' ','Error in HELENAR:  IRET =',I12,
