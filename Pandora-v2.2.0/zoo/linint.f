...
C     (based on "LININT" (1971 Aug 06) and "SLGINTX" (1973 Apr 10),
C     originally written for CDC 6400)
C
...
C     !BEG
      if(NT.gt.1) then
        NTX = 1+INCX*(NT-1)
        NTF = 1+INCF*(NT-1)
        call SEARCH   (XT,INCX,NT,X,K,NOTE,L)
        if(L.eq.1) then
          IRET = 1
          KX = 1+INCX*(K-1)
          KF = 1+INCF*(K-1)
          if(NOTE.eq.1) then
            ...
            call LINT (XT(KX),     FT(KF),
     $                 XT(KX+INCX),FT(KF+INCF),MODE,X,F)
          else
            write (*,100) 'NOTE', NOTE
  100       format(' ','Error in LININT:  ',A,' =',I12)
            call ABORT
          end if
        else if(L.eq.2) then
          IRET = 1
          ...
          if(KODE.eq.1) then
            ...
            call LINT (XT(NTX-INCX),FT(NTF-INCF),
     $                 XT(NTX),     FT(NTF),     MODE,X,F)
          else
