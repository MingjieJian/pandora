...
C     !DASH
      save
C     !DASH
      real*4 A, ARG, BPROF, CPROF, CRIT, DDL, DL, FWHM, SNORM, SUM,
     $       TRTLN2, W1, ZERO
...
C
      data ZERO,TRTLN2,CRIT /0.E0, 1.665109222E0, 1.E20/
C
C     !BEG
      A = TRTLN2/FWHM
      do 102 J = 1,K
  ...
  100   continue
        ...
        M = IE-IS+1
        call HARTR (M, DL(IS), W1(IS), SNORM)
        do 101 I = IS,IE
          W1(I) = CPROF(I)*W1(I)
  101   continue
        call HARTR (M, DL(IS), W1(IS), SUM)
        BPROF(J) = SUM/SNORM
  102 continue
C     !END
C
